...
const CreateThread = ({user}: {user: UserInfoTypes}) => {
  ...
  return (
    <ModalCreateThread user={user}>
      <div className="w-full flexx pb-4 border-b border-gray-200 dark:border-[#222] mb-10" onClick={() => !user?.onboarded && showMsgToast()}>
        ...
        <div className="flexx flex-1">
          ...
          <button className="button-outline">Kirim</button>
        </div>
      </div>
    </ModalCreateThread>
  )
